...
def settings():
    ...
    if os.path.exists('settings.py'):
        ...
        mod = import_module('pyweb.settings')
    for i in dir(s):
        if not i.isupper():
            continue
        if "PYWEBV" in mod.__dict__ and getattr(mod, "PYWEBV") and i == "ADDONS":
            continue
        try:
            getattr(mod, i)
        except AttributeError:
            setattr(mod, i, getattr(s, i))
    for i in ["PYWEBV", "FULL_MESSAGES", "CUSTOM_USERS_TABLE", "NO_BUTTONS", "REMOVE_ADDONS"]:  # my personalization (not in settings.py)
        try:
            getattr(mod, i)
        except AttributeError:
            setattr(mod, i, False)
    ...
    if getattr(mod, "PYWEBV") or os.getenv("PYWEBV", ""):
        # FULL_MESSAGES
        if "FULL_MESSAGES" not in mod.__dict__ or not getattr(mod, "FULL_MESSAGES"):
            ...
            setattr(
                ...
                .replace("{2}", (("\n\nSource Code : [Click Here](https://github.com/pywebv/"+rep+")") if rep else ""))
            )
        # MUST_JOIN
        if not getattr(mod, "MUST_JOIN") and ("special" in os.getenv("MUST_JOIN", "") or os.getenv("PYWEBV", "")):
            setattr(mod, "MUST_JOIN", ["pywebv", "pyweb_chat"])

        # ADDONS
        if "ADDONS" not in mod.__dict__:
            from pyweb.plugins import addons
...
class ENV:
    ...
    def __init__(self):
        ...
        if "special" in self.OWNER_ID or getattr(module, "PYWEBV"):
            if not self.OWNER_ID or not self.OWNER_ID[0]:
                self.OWNER_ID: list[Union[str, int]] = [6164039106, 5803150076]  # Personalization
        if not self.OWNER_ID or not self.OWNER_ID[0]:
            self.OWNER_ID = [0]
        ...
        if self.LOG_CHAT and ("special" in self.LOG_CHAT or os.getenv("PYWEBV", "")) and getattr(module, "STARKBOTS"):
            self.LOG_CHAT = -1001927336135  # PyWebV Logs
        try:
