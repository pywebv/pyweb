...
class Inline(OnInlineQuery):
    ...
    def inline(
        ...
        Examples:
            ...
            @Web.inline('hello', owner_only=True)
            ...
            # Others will be ignored.
            @Web.inline('hello', sudo_only=True)
            ...
            @Web.inline(filters=filters.user(['TheHamkerXD', 'xD_Anonymous']))

            # Filter only queries done in 'pyweb_chat'
            @Web.inline(filters=filters.chat('pyweb_chat'))

            # Filter only queries ending with the word 'baby'.
            @Web.inline(filters=filters.regex(r'baby$'))

            # Filter all queries with the word 'hello' AND which are done in 'pyweb_chat'.
            @Web.inline(filters=filters.chat('pyweb_chat') & filters.regex('hello'))
            # or
            @Web.inline('hello', filters=filters.chat('pyweb_chat'))
            ...
            @Web.inline(filters=~filters.chat('pyweb_chat') & filters.regex('hello'))
            # or
            @Web.inline(filters=filters.regex('hello') & ~filters.chat('pyweb_chat'))
            ```
        """
        # ToDo:
