...
class Command(OnMessage):
    ...
    def command(
        ...
        You can also use the alias ``Web.cmd`` instead of ``Web.command``.
        ...
        Examples:
            ...
            @Web.command(extra_filters=filters.media)

            # Filter only text messages.
            @Web.command(extra_filters=filters.text)

            # Filter only messages sent by 'TheHamkerXD'.
            @Web.command(extra_filters=filters.user('TheHamkerXD'))

            # Filter only messages sent in 'pyweb_chat'
            @Web.command(extra_filters=filters.chat('pyweb_chat'))

            # Filter only messages with the word 'baby'.
            @Web.command(extra_filters=filters.regex('baby'))

            # Filter all media messages sent by bots.
            @Web.command(extra_filters=filters.bot & filters.media)

            # Filter all messages sent by bots except media messages.
            @Web.command(extra_filters=filters.bot & ~filters.media)

            # Filter either media messages or text messages.
            @Web.command(extra_filters=filters.text | filters.media)
            ```
        """
