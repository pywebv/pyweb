...
from pyweb.config import settings, ENV
from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton, Message
from pyrogram.errors import ChatAdminRequired, UserNotParticipant, ChatWriteForbidden


@Web.cmd(private_only=True, extra_filters=~filters.edited & filters.incoming, group=-1)
async def must_join_channel(bot: Web, msg: Message):
    if ENV.DATABASE_URL:
    ...
    try:
        ...
        chats = []
    if not chats:
        Web.log("Addon must_join is loaded but no chat is listed in MUST_JOIN var of settings", "warn")
        return
    nope = []
    for chat in chats:
        try:
            ...
            Web.log(f"I'm not admin in the {chat} Chat !", "warning")
    if not nope:
        return
    buttons = []
    for i in nope:
